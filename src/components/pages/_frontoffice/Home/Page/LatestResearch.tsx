...
const LatestResearch = () => {
    ...
    useEffect(() => {
        async function _getsResearch() {
            await dispatch(getsResearch({ filter: { Status: 'Publish' }, pagination: { page: 1, perPage: 5 } }));
        }
                                    ...
                                    <SwiperSlide key={`slider-${index}`}>
                                        ...
                                        >
                                            <div className="bg-[#F2F2F8] p-2 rounded-full max-w-[50%] text-center mb-[15px] text-xs">
                                                {value?.DataResearchScope?.Name}
                                            </div>

                                            <h3 className="text-black text-[20px] md:text-[22px] font-semibold mb-[10px]">
                                                {(value.Title && value.Title.toString().length >= 20) ? value.Title.toString().substring(0, 50) + " ..." : value.Name}
                                            </h3>
                                            <p>
                                                {(value.Executor && value.Executor.toString().length >= 20) ? value.Executor.toString().substring(0, 50) + " ..." : value.Executor}
                                            </p>

                                            <div className="absolute bottom-6">
                                                <Link
                                                    to={`/research_result/${value.id}`}
